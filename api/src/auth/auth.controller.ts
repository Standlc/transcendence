...
export class AuthController {
   ...
   */
  @ApiOperation({
    summary: 'Redirect user here to redirect them to the 42 API',
  })
  @ApiDefaultResponse({
    description: 'Nothing to return, the user will be redirected to the 42 API',
  })
  @Get('oauth')
   ...
   */
  @ApiOperation({
    summary: 'This handle the redirect of 42 API, do not use directly',
  })
  @ApiQuery({
    name: 'code',
    required: true,
    description:
      'User token of 42 API. Note: This endpoint should not be use directly, as you will never get a valid token for the 42 API. This endpoint purpose is solely for the 42 API.',
  })
  @ApiOkResponse({
    description:
      'This will redirect to either localhost:3000/ if a jwt as been generated, or localhost:3000/login?code=x if something went wrong. Note that x will be replaced with a string of the error.',
  })
  @Get('redirect')
  @UseGuards(OauthGuard)
  ...
  @ApiCreatedResponse({
    description:
      'User logged in, a cookie will be added with a jwt token, if the user activate 2FA, it will return {"isTwoFactorAuthenticationEnabled": true}.',
    schema: {
      type: 'object',
      example: {
        avatarUrl: null,
  ...
  @ApiOkResponse({
    description: 'Return a string containing a qrcode PNG in base64 format',
  })
  @ApiInternalServerErrorResponse({
    description:
      'Whenever the backend fail in some point, probably an error with the db.',
  ...
  async activateTwoFactorAuthentication(@Req() req): Promise<string> {
    const otpAuthUrl: string =
      await this.authService.generateTwoFactorAuthenticationSecret(req.user.id);
  ...
  @ApiOperation({
    summary: 'Turn on the 2FA if the code generate with the qrcode was valid',
  })
  ...
  async turnOnTwoFactorAuthentication(
    ...
    @Req() req,
    @Query('code', new ZodValidationPipe(z.string().length(6)))
  ...
  @ApiUnauthorizedResponse({
    description: '2FA is not enabled or Id was missing from the payload',
  })
  @ApiInternalServerErrorResponse({
    description:
      'Whenever the backend fail in some point, probably an error with the db.',
